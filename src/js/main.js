...
async function displayHolidays(year, countryCode) {
    ...
    holidays.map(holiday => {
        ...
        holidayDate = date.toLocaleString("en-US", { month: "short", day: 'numeric', year: "numeric" })

        let types = holiday.types.map(type => `<span class="holiday-type-badge">${type}</span>`).join('');
...
async function getEvents(city, countryCode) {
    let response = await fetch(`https://app.ticketmaster.com/discovery/v2/events.json?apikey=VwECw2OiAzxVzIqnwmKJUG41FbeXJk1y&city=${city}&countryCode=${countryCode}&size=20`);
    let data = await response.json();
...
function getWeatherIcon(code) {
    ...
    if ([3, 45, 48, 51, 53, 55, 61, 63, 65, 71, 73, 75, 80, 81, 82, 95, 96, 99].includes(code)) return "fa-cloud";
    return "fa-sun";
}
...
async function showWeather(city, countryCode) {
    ...
    for (let i = 0; i < 7; ++i) {
        ...
        let precip = data.daily.precipitation_probability_max[i] ? `<div class="forecast-precip">
...
async function getLongWeekend(year, countryCode) {
    let response = await fetch(`https://date.nager.at/api/v3/LongWeekend/${year}/${countryCode}`);
    let data = await response.json();
    return data;
}
...
holidayContent.addEventListener("click", (e) => {
    ...
    allPlans.push({ type: "holiday", Id: id, Title: title, Name: name, Date: date });
    savePlansToLocalStorage();
...
eventContent.addEventListener("click", (e) => {
    ...
    allPlans.push({ type: "event", Id: id, Name: name, Date: date, Location: location });
    savePlansToLocalStorage();
    loadPlansFromLocalStorage();
})
...
lwContent.addEventListener("click", (e) => {
    ...
    allPlans.push({ type: "longWeekend", Id: id, Days: days, Date: date, Extra: extra });
    savePlansToLocalStorage();
    loadPlansFromLocalStorage();
